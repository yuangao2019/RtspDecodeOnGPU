...
static int CUDAAPI HandleVideoSequence(void* pUserData, CUVIDEOFORMAT* pFormat) {
	...
	if ((pFormat->codec != pDecoder->m_oVideoDecodeCreateInfo.CodecType) ||         // codec-type
		...
		real_width = pFormat->coded_width;
		real_height = pFormat->coded_height;
		return 1;
	}
...
bool CudaDecoder::mapFrame(int iPictureIndex, CUdeviceptr* ppDevice, unsigned int* pPitch,
...
void
CudaDecoder::YUV420P2BGR32(unsigned char* yuv_buffer_in, unsigned char* rgb_buffer_out, unsigned int pitch, int height,
                           int width) {
...
void
CudaDecoder::yuv420p2bgr(CUdeviceptr yuv_buffer_in, cv::cuda::GpuMat& rgb_buffer_out, unsigned int pitch, int width,
                           int height) {
	yuv420p2bgr32(yuv_buffer_in, rgb_buffer_out, pitch, tHeight, tWidth, height, width);
}
